...
public class CameraProcessing : MonoBehaviour {
	...
	void Update() {
		...
		if (frame.Images[0].Width == 0 || frame.Images[0].Height == 0) {
			Debug.Log("No data in the images.");
			return;
		}
		
		//IF IMAGE DIMENSIONS HAVE CHANGED, CHANGE BYTE ARRAY SIZE
		if (frame.Images[0].Width != Width || frame.Images[0].Height != Height) {
			Width = frame.Images[0].Width;
			Height = frame.Images[0].Height;
			SetDimensions(Width, Height);
		}
		
		//PUT IMAGES INTO BYTE ARRAYS
		r_image_data = frame.Images[0].Data;
		l_image_data = frame.Images[1].Data;
		...
		
		{
			...
			for (int i = 0; i<rcount*3; i=i+3) {
				...
				Vector fBlob = frame.Images [0].Rectify (new Vector (RBlobs [i] / RBlobs [i + 2], RBlobs [i + 1] / RBlobs [i + 2], 0));
			...
			for (int i = 0; i<lcount*3; i=i+3) {
				Vector fBlob = frame.Images [1].Rectify (new Vector (LBlobs [i] / LBlobs [i + 2], LBlobs [i + 1] / LBlobs [i + 2], 0));
				...
				if (RBlobs [i + 2] < BlobSizeCutoff) {
					...
					for (int j = 0; j<lcount*3; j=j+3) {
						if (LBlobs [j + 2] < BlobSizeCutoff) {
							...
							float distancefromleap = ClosestTimeOfApproach (CameraOffset, RDir, -CameraOffset, LDir) * -1f;
							if (DrawDebug) {
			...
			for (int i = 0; i<rcount; i++) {
				if ((MatchBlobs [i] != 256)) {
					...
					Blobs.Add (transform.TransformPoint (ClosestPointOfApproach (CameraOffset, LDir, -CameraOffset, RDir)));
				}
			}
